...
async def main(args):
    ...
    # 评估四种不同的负载模式
    load_patterns = ["socialnetwork_typeA", "socialnetwork_typeB", "socialnetwork_typeC", "socialnetwork_typeD"]

    # 用户数量范围
    user_counts = range(50, 451, 50)  # 50到450，步长50

    for load_pattern in load_patterns:
        print(f"\n========== 开始评估负载模式: {load_pattern} ==========")

        # 为当前负载模式创建专用文件夹
        load_data_path = os.path.join(base_eval_data_path, f"load={load_pattern}")
        if not os.path.exists(load_data_path):
            os.makedirs(load_data_path)

        # 创建负载模式的汇总结果文件
        load_summary_path = os.path.join(load_data_path, "load_summary.csv")
        with open(load_summary_path, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([
                'user_count', 'avg_reward', 'std_reward', 'avg_latency_90', 'std_latency_90', 'avg_latency_95',
                'std_latency_95', 'avg_latency_99', 'std_latency_99'
            ])
        ...
        for user_count in user_counts:
            ...
            # 为当前用户数量创建专用文件夹
            eval_data_path = os.path.join(load_data_path, f"users={user_count}")
            ...
            try:
                ...
                # 记录当前用户数量的评估结果到负载模式汇总文件
                with open(load_summary_path, 'a', newline='') as f:
                    writer = csv.writer(f)
                    writer.writerow([
                        user_count,
                        np.mean(all_total_rewards),
                        np.std(all_total_rewards),
                        np.mean(all_latency_metrics['90%']),
                        np.std(all_latency_metrics['90%']),
                        np.mean(all_latency_metrics['95%']),
                        np.std(all_latency_metrics['95%']),
                        np.mean(all_latency_metrics['99%']),
                        np.std(all_latency_metrics['99%'])
                    ])
