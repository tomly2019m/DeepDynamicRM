...
def parse_args():
    ...
    parser.add_argument('--target-update-interval',
                        type=int,
                        default=1000,
                        help='目标网络更新周期（每隔多少步更新）(默认: 1000)')
    parser.add_argument('--replay-size',
                        type=int,
                        default=int(1e6),
                        help='经验回放池容量 (默认: 1,000,000)')
    ...
    parser.add_argument('--random-steps',
                        type=int,
                        default=200,
                        help='初始随机探索步数，不更新策略网络 (默认: 200)')
    parser.add_argument('--exploration-steps',
                        type=int,
                        default=45 * 200,
                        help='e-greedy探索总步数，含随机探索阶段 (默认: 9000)')

    # 训练终止参数
    parser.add_argument('--stop-steps',
                        type=int,
                        default=20 * 3600,
                        help='训练最大总步数，超过此值停止训练 (默认: 72000)')
    # 运行模式参数
    parser.add_argument('--train',
                        action='store_true',
                        help='启用训练模式（不指定则默认验证模式）')
