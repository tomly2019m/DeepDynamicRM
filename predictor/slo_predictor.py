...
def process_data(window_size=10, pred_window=5, threshold=500, save_scalers=True):
    """
    处理时序数据生成带滑动窗口的训练数据集
...
def train_example(
    ...
    def print_shape_info(name, data):
        print(f"{name}服务数据: {data[0].shape} | 延迟数据: {data[1].shape} | 标签: {data[2].shape}")
...
def predict_example():
    ...
    def case_normal():
        ...
        with torch.no_grad():
            prob = model(
                torch.FloatTensor(processed_serv), 
                torch.FloatTensor(processed_lat)
            ).item()
