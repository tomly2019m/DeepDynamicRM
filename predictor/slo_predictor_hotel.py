...
# 加载三个数据集
gathered = np.load(f"{data_dir}/gathered.npy")  # 形状 (n,17,6,4)
latency = np.load(f"{data_dir}/latency.npy")  # 形状 (n,6)
replicas = np.load(f"{data_dir}/replicas.npy")  # 形状 (17,)
cpu_configs = np.load(f"{data_dir}/cpu_config.npy")  # 形状（n,17）
...
# 检查服务数量一致性
assert gathered.shape[1] == replicas.shape[0] == 17, \
...
def process_gathered(gathered, replicas):
    ...
    # 展平最后两个维度 (n,17,6,4) → (n,17,24)
    flattened = gathered.reshape(n_timesteps, 17, -1)

    replicas = np.log1p(replicas)  # 加1避免log(0)

    cpu_configs_expanded = cpu_configs[:, :, np.newaxis].astype(np.float32)  # 添加新轴 → (n,17,1)

    # 直接广播原始副本数（不标准化）到所有时间步：(17,) → (n,17,1)
    replicas_expanded = np.tile(
        replicas.reshape(1, -1, 1),  # 先变形为 (1,17,1)
        (n_timesteps, 1, 1)  # 沿时间步复制n次 → (n,17,1)
...
def process_data(window_size=30, pred_window=5, threshold=200, save_scalers=True):
    """
    处理时序数据生成带滑动窗口的训练数据集
    ...
    threshold (int): SLO违例阈值(单位:ms)，默认200ms，超过则标记为违例
...
class ServiceBranch(nn.Module):
    ...
    def __init__(
        ...
        if 'hier' in mode:
            ...
            self.fusion = nn.Sequential(nn.Linear(2 * lstm_hidden * 16, 512), nn.ReLU(), nn.Linear(512, lstm_hidden))
