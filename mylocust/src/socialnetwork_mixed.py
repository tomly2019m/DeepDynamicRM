...
class LoadConfig:
    # 阶段持续时间（秒）
    PHASE_DURATION = 500  # 每个负载阶段持续500秒
    CYCLE_DURATION = PHASE_DURATION * 4  # 完整循环2000秒

    # 用户增长配置
    USER_INCREMENT = 50  # 每轮循环增加用户数
...
class GlobalState:
    ...
    def get_phase(self):
        ...
        phase_index = int(
            (elapsed % LoadConfig.CYCLE_DURATION) // LoadConfig.PHASE_DURATION)
    ...
    def update_users(self, env: Environment):
        with self._lock:
            # 每完整循环增加用户数
            full_cycles = int(
                (time.time() - self.test_start) // LoadConfig.CYCLE_DURATION)
            if full_cycles > self.cycle_count:
                ...
                if env.runner and not isinstance(env.runner, WorkerRunner):
                    # env.runner.stop()
                    env.runner.start(user_count=new_count,
                                     spawn_rate=new_count // 10)
...
class SocialMediaUser(HttpUser):
    ...
    def _daynight_wait(self):
        ...
        current_iat = cfg["base_iat"] + (cfg["peak_iat"] -
                                         cfg["base_iat"]) * (1 - factor)
    ...
    def _get_burst_window(self, cycle_num):
        ...
        if cycle_num not in self._burst_schedule:
            prev_end = self._burst_schedule.get(cycle_num - 1,
                                                {}).get('end', 0)
            min_start = max(
                0, prev_end + cfg["min_interval"] - cfg["cycle_duration"])
            start = random.uniform(
                min_start, cfg["cycle_duration"] - cfg["burst_duration"])
            self._burst_schedule[cycle_num] = {
                'start': start,
    ...
    def _noise_wait(self):
        ...
        # 组合噪声生成
        noise = self._gaussian_noise() + self._impulse_noise(
        ) + self._random_walk()
    ...
    def _gaussian_noise(self):
        ...
        a, b = (cfg["clip"][0] - cfg["mu"]) / cfg["sigma"], (
            cfg["clip"][1] - cfg["mu"]) / cfg["sigma"]
    ...
    def _random_walk(self):
        ...
        self._last_iat = cfg["persistence"] * self._last_iat + (
            1 - cfg["persistence"]) * delta
    ...
    def compose_post(self):
        ...
        for i in range(0, 5):
            if random.random() <= 0.2:
                ...
                for i in range(0, num_urls):
                    text = text + " https://www.bilibili.com/av" + random_decimal(
                        8)
        ...
        if r.status_code > 202:
            logging.warning('compose_post resp.status = %d, text=%s' %
    ...
    def read_home_timeline(self):
        ...
        if r.status_code > 202:
            logging.warning('read_home_timeline resp.status = %d, text=%s' %
                            (r.status_code, r.text))
    ...
    def read_user_timeline(self):
        ...
        if r.status_code > 202:
            logging.warning('read_user_timeline resp.status = %d, text=%s' %
                            (r.status_code, r.text))
...
def on_locust_init(environment: Environment, **kwargs):
    if not isinstance(environment.runner, WorkerRunner):
        # 从命令行获取初始用户数
        global_state.user_count = environment.parsed_options.num_users or 50
        print(f"解析到用户数量{global_state.user_count}")

        # 启动状态监控线程
        def monitor():
            while True:
                global_state.update_users(environment)
                time.sleep(10)
